import { motoko_test5 } from "../../declarations/motoko_test5";
...
async function post() {
  ...
  try {
      await motoko_test5.post(otp, text);
      textarea.value = "";
  } catch (error) {
...
async function load_posts() {
  ...
  let posts = await motoko_test5.posts(0);
...
async function set_name() {
  ...
  try {
      await motoko_test5.set_name(name_input.value);
      name_input.value = "";
  } catch (error) {
      alert(error);
  }
...
async function load_follows() {
  let follows = await motoko_test5.follows();
  ...
  for (let i = 0; i < follows.length; i++) {
      let nameBy = await motoko_test5.nameBy(follows[i].toString());
